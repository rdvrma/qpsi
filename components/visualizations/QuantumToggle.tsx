...
export function QuantumToggle() {
  ...
  return (
    <div className="w-full space-y-8">
      {/* Toggle Controls */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pb-4 border-b border-white/14">
        <div className="flex items-center space-x-2 bg-[#0E0E0E] p-1 border border-white/14 w-full sm:w-auto">
          <button
            ...
            className={`flex-1 sm:flex-none px-6 py-2.5 text-xs font-mono font-semibold uppercase tracking-wider transition-all ${
              activeTab === 'classical'
                ? 'bg-primaryWhite text-bgBlack shadow-md'
        ...
        <div className="text-xs font-mono text-midGray">
          Active Execution Path: <span className="text-primaryWhite font-semibold">{currentData.mode}</span>
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-6 border-b border-white/10 gap-4">
            ...
            <span className="px-3 py-1 border border-white/20 text-xs font-mono uppercase text-softWhite bg-white/[0.02]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-2">
            {currentData.metrics.map((m, idx) => (
              <div
                key={idx}
                className="p-4 border border-white/10 bg-[#0E0E0E] space-y-1 hover:border-white/20 transition-colors"
              >
                <div className="text-[10px] font-mono uppercase tracking-wider text-midGray">
                  {m.label}
                </div>
                <div className="text-sm font-mono font-semibold text-primaryWhite">
                  {m.value}
                </div>
              </div>
            ))}
      ...
      <div className="p-6 border border-white/20 bg-[#040404] space-y-3">
        ...
        <div className="text-xs font-mono text-softWhite whitespace-pre-line leading-relaxed pl-7">
